...
class App(tk.Tk):
    def __init__(self):
        ...
        #=================================================หน้า แลกของ=====================================================#
        self.point_products = [
            {"shop" : "SHOP1","name": "เข็มขัด", "point": 5},
            {"shop" : "SHOP1","name": "เสื้อ", "point": 5},
            {"shop" : "SHOP1","name": "กางเกง", "point": 5},
            {"shop" : "SHOP1","name": "เข็มขัด", "point": 5},
            {"shop" : "SHOP1","name": "เข็มขัด", "point": 5},
            {"shop" : "SHOP2","name": "เข็มขัด", "point": 5},
            {"shop" : "SHOP2","name": "เข็มขัด", "point": 5},
            {"shop" : "SHOP2","name": "เข็มขัด", "point": 5},
            {"shop" : "SHOP2","name": "เข็มขัด", "point": 5},
            {"shop" : "SHOP2","name": "เข็มขัด", "point": 5},
    ...
    def display_cart_point(self):
        ...
        for shop_name, items in cart_by_shop.items():
            ...
            confirm_button = tk.Button(self.point_kart_frame, text="ชำระผ่านบัตร", bg="#4C956C", fg="white", width=15, height=1, border=0, font=('Browallia New', 12, 'bold'), command=lambda total_points=total_points, cart_items=cart_counts.items(): self.confirm_purchase_point(total_points, cart_items, uid,shop_name))
            confirm_button.grid(row=row_offset, column=0, padx=30, pady=10, sticky="e")
            row_offset += 1

    def confirm_purchase_point(self, total_points, cart_items, card_owner_uid,shop_name):
        self.shopping_cart.clear()
        self.display_cart()
        ...
        shop_label = tk.Label(new_window, text=f"ร้านของ : {shop_name}", font=('Browallia New', 16),bg='white')
        shop_label.place(x=0, y=0)
        ...
        otp_input.place(x=520,y=450)
        otp_input.insert(0,'กรุณากรอก OTP')
        tk.Frame(new_window,width=100,height=2,bg='black').place(x=450,y=560)
        
        get_otp_button = tk.Button(new_window, text="ขอ OTP", bg="#ffa500", fg="white", width=10, height=1, border=0, font=('Browallia New', 12, 'bold'),  command=lambda: self.send_email_with_otp1(uid))
        get_otp_button.place(x=660, y=450)
    ...
    def redeem_points(self,uid, shop , total_points):
        filename = 'data.json'
        try:
            with open(filename, 'r') as file:
                data = json.load(file)
        ...
        for user in data['USER']:
            if user['UID'] == uid:
                for s in data[shop]:
                    if s['UID'] == uid:
                        if shop == 'SHOP1':
                            if s['Point'] >= total_points:
                                s["Point"] -= total_points
                                user_found = True
                            else:
                                user_found = True
                                print("Point ไม่เพียงพอ")
                        elif shop == 'SHOP2':
                            if s['Point'] >= total_points:
                                s["Point"] -= total_points
                                user_found = True
        ...
        if not user_found:
            print("ไม่พบ UID หรือไม่พบข้อมูลร้าน")
        else:
            with open(filename, 'w') as file:
                json.dump(data, file, indent=4)
    
    def verify_otp_point(self,uid, shop , total_points,otp_input):
            entered_otp = otp_input.get()
            if entered_otp == self.otp:
                messagebox.showinfo("Success", "OTP Verified Successfully!")
                self.redeem_points(uid, shop , total_points)
            else:
                messagebox.showerror("Error", "Invalid OTP. Please try again.")
    
    
    def send_email_with_otp1(self,uid):
        sender_email = "nopanon.ve@ku.th"
        password = "Nope$$2945"
        UID = uid
    ...
    #========================================================== function ได้แต้ม ==============================================#
    def buyitem(self,buy, uid, shopno):
        filename = 'data.json'
...
# print("ดึงข้อมูลสำเร็จ")
    ...
    #     try:
    #         with open(remote_filename, 'wb') as localfile: 
    #             self.ftp.retrbinary(f'RETR {remote_filename}', localfile.write, 1024)
